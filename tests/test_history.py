...
class TestBuildSnapshot:
    ...
    def test_public_api_build_snapshot(self):
        """공개 API build_snapshot이 정상 동작해야 한다."""
    ...
    def test_with_ecosystem(self):
        ...
        eco.compute_score()
        snap = build_snapshot(results, eco)
...
class TestComputeDiff:
    ...
    def test_same_snapshot(self):
        results = _make_results()
        baseline = build_snapshot(results, evaluator_version="v-old")
        current = build_snapshot(results, evaluator_version="v-old")
    ...
    def test_improved(self):
        results = _make_results()
        baseline = build_snapshot(results)
        ...
        lr2.compute_score()
        current = build_snapshot({"alpha": {"L1": lr2}})
    ...
    def test_new_skill(self):
        ...
        current = build_snapshot(current_results)
    ...
    def test_evaluator_version_changed(self):
        results = _make_results()
        baseline = build_snapshot(results, evaluator_version="abc123")
        current = build_snapshot(results, evaluator_version="def456")
        diff = compute_diff(current, baseline)
